...
import Sample01.kis_auth as kis
...
def get_order_cash(
    ord_dv="", itm_no="", qty=0, unpr=0, tr_cont="", FK100="", NK100="", dataframe=None
):  # 국내주식주문 > 주식주문(현금)
...
def get_order_rvsecncl(
    ...
    tr_id = (
        "TTTC0803U"  # 주식 정정 취소 주문    [모의투자] VTTC0803U : 주식 정정 취소 주문
    )
    ...
    params = {
        ...
        "ORD_QTY": str(
            int(ord_qty)
        ),  # 주문주식수 	[잔량전부 취소/정정주문] "0" 설정 ( QTY_ALL_ORD_YN=Y 설정 ) [잔량일부 취소/정정주문] 취소/정정 수량
        "ORD_UNPR": str(
            int(ord_unpr)
        ),  # 주문단가 	[정정] 정정주문 1주당 가격 [취소] "0" 설정
...
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output API 문서 참조 등
def get_inquire_psbl_rvsecncl_lst(
    tr_cont="", FK100="", NK100="", dataframe=None
):  # 국내주식주문 > 주식정정취소가능주문조회
    ...
    )  # 페이징 처리 getHeader(), getBody() kis_auth.py 존재
...
def get_inquire_daily_ccld_obj(
    dv="01",
    inqr_strt_dt=None,
    inqr_end_dt=None,
    tr_cont="",
    ...
    if inqr_strt_dt is None:
        inqr_strt_dt = datetime.today().strftime(
            "%Y%m%d"
        )  # 시작일자 값이 없으면 현재일자
    if inqr_end_dt is None:
        inqr_end_dt = datetime.today().strftime(
            "%Y%m%d"
        )  # 종료일자 값이 없으면 현재일자
...
def get_inquire_daily_ccld_lst(
    dv="01",
    inqr_strt_dt="",
    inqr_end_dt="",
    tr_cont="",
    ...
    if tr_cont == "D" or tr_cont == "E":  # 마지막 페이지
        ...
        print("Call Next")
        return get_inquire_daily_ccld_lst(
            dv, inqr_strt_dt, inqr_end_dt, "N", FK100, NK100, dataframe
        )
...
def get_inquire_psbl_order(
    pdno="", ord_unpr=0, tr_cont="", FK100="", NK100="", dataframe=None
):  # 국내주식주문 > 매수가능조회
...
def get_order_resv_rvse(
    ...
    params = {
        ...
        "ORD_UNPR": str(
            int(ord_unpr)
        ),  # 주문단가 	[정정] 1주당 가격 * 장전 시간외, 시장가의 경우 1주당 가격을 공란으로 비우지 않음 "0"으로 입력 권고
...
def get_order_resv_ccnl(
    ...
    if tr_cont == "D" or tr_cont == "E":  # 마지막 페이지
        ...
        return get_order_resv_ccnl(
            inqr_strt_dt, inqr_end_dt, ord_seq, "N", FK100, NK100, dataframe
        )
...
def get_inquire_balance_rlz_pl_obj(
    tr_cont="", FK100="", NK100="", dataframe=None
...
def get_inquire_balance_rlz_pl_lst(
    tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 주식잔고조회_실현손익 (보유주식내역 Output2)
    url = "/uapi/domestic-stock/v1/trading/inquire-balance-rlz-pl"
    tr_id = "TTTC8494R"
...
def get_inquire_credit_psamount(
    pdno="", ord_unpr="", tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 신용매수가능조회
...
def get_inquire_period_trade_profit_obj(
    inqr_strt_dt=None, inqr_end_dt=None, tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 기간별매매손익현황내역 (output2)
    ...
    current_data = pd.DataFrame(
        res.getBody().output2, index=[0]
    )  # getBody() kis_auth.py 존재
    dataframe = current_data
    return dataframe
...
def get_inquire_period_trade_profit_lst(
    inqr_strt_dt=None, inqr_end_dt=None, tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 기간별매매손익현황조회 (output1)
    url = "/uapi/domestic-stock/v1/trading/inquire-period-trade-profit"
    tr_id = "TTTC8715R"
    ...
    if tr_cont == "D" or tr_cont == "E":  # 마지막 페이지
        ...
        print("Call Next")
        return get_inquire_period_trade_profit_lst(
            inqr_strt_dt, inqr_end_dt, "N", FK100, NK100, dataframe
        )
...
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output2 - 매도수량합계, 매도거래금액합계, 매도수수료합계, 매도제세금합계, 매도정산금액합계, 매수수량합계... 등
def get_inquire_period_profit_obj(
    inqr_strt_dt=None, inqr_end_dt=None, tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 기간별손익일별합산조회 (output2)
...
def get_inquire_period_profit_lst(
    inqr_strt_dt=None, inqr_end_dt=None, tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 주문/계좌 > 기간별손익일별합산조회 (output1)
    url = "/uapi/domestic-stock/v1/trading/inquire-period-profit"
    tr_id = "TTTC8708R"
    ...
    if tr_cont == "D" or tr_cont == "E":  # 마지막 페이지
        ...
        print("Call Next")
        return get_inquire_period_profit_lst(
            inqr_strt_dt, inqr_end_dt, "N", FK100, NK100, dataframe
        )
...
def get_inquire_price(
    div_code="J", itm_no="", tr_cont="", FK100="", NK100="", dataframe=None
):  # [국내주식] 기본시세 > 주식현재가 시세
...
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output
def get_inquire_asking_price_exp_ccn(
    output_dv="1",
    div_code="J",
    itm_no="",
    tr_cont="",
    ...
    if output_dv == "1":
        current_data = pd.DataFrame(
            res.getBody().output1, index=[0]
        )  # 호가조회  * getBody() kis_auth.py 존재
    else:
...
def get_inquire_daily_itemchartprice(
    ...
    if inqr_strt_dt is None:
        inqr_strt_dt = (datetime.now() - timedelta(days=14)).strftime(
            "%Y%m%d"
        )  # 시작일자 값이 없으면 현재일자
    if inqr_end_dt is None:
        inqr_end_dt = datetime.today().strftime(
...
def get_inquire_daily_itemchartprice(
    ...
    if inqr_strt_dt is None:
        inqr_strt_dt = (datetime.now() - timedelta(days=100)).strftime(
            "%Y%m%d"
        )  # 시작일자 값이 없으면 현재일자
    if inqr_end_dt is None:
        inqr_end_dt = datetime.today().strftime(
...
def get_inquire_daily_overtimeprice(
    ...
    if output_dv == "1":
        current_data = pd.DataFrame(
            res.getBody().output1, index=[0]
        )  # 시간외 현재가  * getBody() kis_auth.py 존재
    else:
        current_data = pd.DataFrame(
            res.getBody().output2
        )  # 일자별 시간외주가 최근 30일
