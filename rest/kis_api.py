...
def _url_fetch(
    api_url, ptr_id, params, appendHeaders=None, postFlag=False, hashFlag=True
):
...
def do_order(
    stock_code, order_qty, order_price, prd_code="01", buy_flag=True, order_type="00"
):
...
def do_sell(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    t1 = do_order(
        stock_code, order_qty, order_price, buy_flag=False, order_type=order_type
    )
    return t1.isOK()
...
def do_buy(stock_code, order_qty, order_price, prd_code="01", order_type="00"):
    t1 = do_order(
        stock_code, order_qty, order_price, buy_flag=True, order_type=order_type
    )
    return t1.isOK()
...
def get_stock_history_by_ohlcv(stock_no, gb_cd="D", adVar=False):
    ...
    hdf1[["Open", "High", "Low", "Close", "Volume"]] = hdf1[
    ...
    if adVar:
        ...
        hdf1["pct_change"] = (
            (hdf1["Close"] - hdf1["Close"].shift(-1)) / hdf1["Close"].shift(-1) * 100
        )
...
def get_stock_investor(stock_no):
    ...
    if t1.isOK():
        ...
        hdf1.rename(columns=ren_dict, inplace=True)
        hdf1[["Date"]] = hdf1[["Date"]].apply(pd.to_datetime)
        hdf1[["PerBuy", "ForBuy", "OrgBuy"]] = hdf1[
            ["PerBuy", "ForBuy", "OrgBuy"]
        ].apply(pd.to_numeric)
